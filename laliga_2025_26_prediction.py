...
def prepare_training_data(season_files: List[str]) -> Tuple[pd.DataFrame, pd.Series, pd.DataFrame]:
    ...
    # incorporate promoted teams for 2025/26 (Levante, Elche, Real Oviedo)
    promoted = ["Levante", "Elche", "Real Oviedo"]
    # if a promoted team already exists in last_summary, use its stats
...
def main():
    # define the season files in chronological order (LaLiga seasons)
    season_files: List[str] = [os.path.join(os.path.dirname(__file__), f"season-{y}.csv") for y in [
    ...
    # LaLiga contains exactly 20 clubs.  Since we may
    ...
    print("Predicted LaLiga 2025/26 table (1 = champion):")
